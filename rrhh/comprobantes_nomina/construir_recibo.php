...
/////////////////////////////////////////////////////////////
  ...
  if (($clave==1) && ($entregar==1))
    {
    	if ($_SESSION['nivel']==3)
    	  $passw_arch=$_SESSION['login_userpass'];
    	else
    	  $passw_arch=$fila['PWD'];
    	$pdf->SetProtection(array('modify','copy'), $passw_arch, null, 0, null);
...
///////////TABLA DE DETALLES//////////////////////////////////////////////////////////////////////
  ...
  $query = $query . " decode(rtrim(ltrim(to_char(saldo,'99,999,990.00'))),'0.00','',to_char(saldo,'999,999,990.00')) as SALDO,";
...
$saldo_pres = "";*/
  ...
  while (($fila2 = oci_fetch_array($ptid, OCI_BOTH)) != false) {
    ...
    $salida = $salida .'</tr>';
		...
		/*if (($entregar==1) && (($tienecorreo==1) || ($tienecorreo==3)))
		{    
		    if ($_SESSION['nivel']==3)
		      $mail=$_SESSION['userid'];
		    
		    $pdf->Output(dirname(__FILE__)."/".$archivopdf, 'F');
		    $result=ENVIAR_CORREO($cuerpo,$asunto,$archivopdf,$mail,1);
		    usleep(15000000);
		}
  */
...
if ($count_reg==0)
	$result = " No hay Registro en este Periodo para este Trabajador";
else{
  if (($entregar==1) && (($tienecorreo==1) || ($tienecorreo==3)))
    {       
        
        $result=ENVIAR_CORREO($cuerpo_mult, $asunto_mult, $archivopdf_mult, $mail_mult, 2);
	...
	{
    	$pdf->Output(dirname(__FILE__)."/".$archivopdf, 'F');
    	$result=ENVIAR_CORREO($cuerpo,$asunto,$archivopdf,$txtdestino,2);
	}
}
//pg_close($conexion);
...
//$result=ENVIAR_CORREO($cuerpo,$asunto,$archivopdf,$mail,$entregar);
//echo $cuerpo.'*'.$asunto.'*'.$archivopdf.'*'.$mail;
