...
//	CargarCombo($("#cboCcosto"),"cargar_combo_db.php?tabla=datos_agr_trab&campo1=DATO&campo2=DESCRIPCION&where=trim(agrupacion)&valor_cond='TRACOST' and trim(descripcion) <> 'S/N'&orderby=DESCRIPCION&firsttext=[Elija Centro Costo]");
	...
	CargarCombo($("#cboTrabajador"),"cargar_combo_db.php?tabla=VW_DATOS_TRAB_SITTWEB_SID&campo1=TRABAJADOR&campo2=NOMBRES&selected=0&orderby=NOMBRES&where=to_number(trabajador)&valor_cond="+$('#hddced').val());

	agregar_trabajador($('#hddced').val());
	agregar_asunto();
...
<form id="formulario" name="formulario" autocomplete="on" method='post'>
    ...
    <div class="input-group">
      ...
      <input  name='hddced' id='hddced' type='hidden' value='<?php echo $_SESSION['ced']; ?>'/>
      <input  name='hddnivel' id='hddnivel' type='hidden' value='<?php echo $_SESSION['nivel']; ?>'/>
      <input  name='hddnombre' id='hddnombre' type='hidden' value='<?php echo $_SESSION['username']; ?>'/>
     
      <span class="input-group-btn">
        <button id="cmdEnviar" class="btn btn-default" type="button" onclick="VerificarPeriodo('enviar');">Enviar</button>
...
<table width="80%">      
    <tr>
    	...
    	<td width="30%">
    		<select onchange="agregar_asunto()" name="cboTnomina" id="cboTnomina" class="form-control">
    			<option value="LA">ANTICIPO (LA)</option>
    			<option selected="" value="LM">MENSUAL (LM)</option>
    			<option value="PA">ANTICIPO PASANTE (PA)</option>
    			<option value="PM">MENSUAL PASANTE (PA)</option>
    			<option value="UT">UTILIDADES (UT)</option>
    			<option value="ES">ESPECIAL (ES)</option>
    		</select>
    	</td>
    	<td width="20%"> &nbsp;</td>
    	<td width="10%"><label>Clase Nomina:</label></td>
    	<td width="30%">
    		<select onchange="agregar_asunto()" name="cboCnomina" id="cboCnomina" class="form-control">
    			<option selected="" value="ME">MENSUAL (ME)</option>
    			<option value="PA">PASANTE (PA)</option>
    		</select>
    	</td>
    </tr>
    <tr>
...
<table width="100%">
	<tr>
		...
		<td width="40%"><input type="radio" name="entrega" checked value="1"><label>Correo Registrado</label><br>
						<input type="radio" name="entrega" value="2"><label>Otro(s):</label>				
		</td>
		<td width="40%"><input type="radio" name="clave"  value="1"><label>Encriptado</label><br>
						<input type="radio" name="clave" checked value="2"><label>Desencriptado</label></td>	
	</tr>
	<tr>
		<td width="10%"> &nbsp;</td>
		<td width="10%"> &nbsp;</td> 
...
?>		
	<tr>
		<td width="10%"><input type="checkbox"  checked="checked" onclick="this.checked=true;"  name="chkTrabajador" id="chkTrabajador"> </td>
		<td width="10%"><label>Trabajador:</label></td>
		<td width="40%"><select name="cboTrabajador" id="cboTrabajador" class="form-control" ></select></td>
		<td width="40%"></td>
	</tr>
	<tr>
		<td width="10%">&nbsp; </td>
		<td width="10%">&nbsp;</td>
		<td width="40%">
			<textarea readonly="" class="form-control" id="txtTrabajador" name="txtTrabajador" rows="2" cols="50" wrap="soft"></textarea>
...
<?php //piedepagina(); 
	...
	header('Location: login/cambio_clave.php');
}	
else{
