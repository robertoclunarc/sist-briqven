...
if (isset($_SESSION['cedula_session_const']) && isset($_SESSION['nivel_const'])){
	...
	if ($tipomovimiento==4){	
      ...
      /************************************************************/
      /* SE VERIFICA SE ESTA TILDADA LA OPCION DE CAMBIO DE TURNO */
      /************************************************************/
...
/************************************************************************************/
/*                        FUNCION PARA CAMBIAR CUADRILLA                            */
/************************************************************************************/
function cambiar_turno($sustituto, $motivo, $finicio, $ffin , $causal, $SH, $SH_anterior){
		$finix = date_create($finicio);
		$fini1 = date_format($finix, 'Y-m-d');
      ...
      $hoy=date('Y-m-d'); 
		while ($fecha_a_crear <= $ffin) {
            #CONSULTAMOS LA ESPERANZA DE ALGUN TRABAJADOR QUE ESTE EN EL SISTEMA HORARIO SELECCIONADO, AGRUPANDOS POR LOS 
            #CAMPOS a.Entrada_Esperada1 as Entrada_Esperada1, a.Salida_Esperada1 as Salida_Esperada1, Y EL QUE TENGA MAYOR NUMERO DE 
            #VECES REPETIDAS (AGRUPADOS) SE TOMA COMO LA ESPERANZA REAL 
			$query1="SELECT top 1 a.Entrada_Esperada1 as Entrada_Esperada1, a.Salida_Esperada1 as Salida_Esperada1, 
			CASE
...
//print $row['Entrada_Esperada1']."*".$row['Salida_Esperada1'];
			...
			$query_insert="INSERT SW_Cambios_de_Esperanza (cedula,fecha,EntradaE1, SalidaE1,EntradaE2, SalidaE2,autorizado) VALUES (".$sustituto.",'".$fecha_a_crear."','".$entEsp1."','".$salEsp1."',".$Entrada_Esperada2.",".$Salida_Esperada2.",".$_SESSION['cedula_session_const'].")";

			$stmt2  = $conn->query($query_insert);
