...
session_start();
  ...
  if ($_SESSION['nivel_const']==2 || $_SESSION['nivel_const']==1){
    ...
    if ($trabajador!='0' and $trabajador!='NULL'){
      ...
      $qry.=" and cast(r.cedula as text) in (SELECT trabajador FROM adam_vw_dotacion_briqven_02_mas WHERE trim(trabajador) in (".llenar_lista_trabajadores_del_supervisor().")) and cast(horas_dlt as float)>0";
    }
  }
  if ($cbostatus==1)
  ...
  function buscar($b) {
    ...
    $contar = ejecutar_num_rows($result);
          ...
          while ($row=ejecutar_fetch_array($result)){
                    ...
                    if ($row["autorizado2"]=='')
                        $inpt .='<td width="5%" style="text-align: center"><input type="checkbox" name="autorizado[]" id="autorizado[]" '.$check.' value="'.$row['cedula']."*".$row['nombre']."*".$row['fecha']."*".$row['entrada_real1']."*".$row['entrada_real2']."*".$row['salida_real1']."*".$row['salida_real2']."*".$row['entrada_esperada1']."*".$row['salida_esperada1']."*".$row['inicio_dlt1']."*".$row['fin_dlt1']."*".$horas_dlt."*".$row['observaciones'].'" '.$disable.'></input></td>'; 
                    else
                    ...
                    $inpt .='<td width="5%"><h6><input type="hidden" name="salida_REAL1[]" value="'.$salida_real.'"></input>'.$row['salida_real1'].'</h6></td>'; 
                    ...
                    $inpt .='<td width="5%" style="text-align: right"><h6><input type="hidden" name="Horas_DLT[]" value="'.$horas_dlt.'"></input>'.$horas_dlt.'</h6></td>'; 
                    ...
                    }elseif ($row["autorizado2"]=='' && $row["rechazado_stdlt"]!=0){
                        $inpt .='<td width="5%"><h6><input type="hidden" name="autorizado2[]" value="'.$row['autorizado2'].'">Rechazada</input></h6></td>';
                    }elseif ($row["autorizado2"]=='' && $row["rechazado_stdlt"]==0){
                ...
                if ($contar>0){ 
                  $inpt .=' </tbody>
                   <tfoot>
                        <tr>
                           <td colspan="18" align="center"> <INPUT id="cmdGuardar" type="button" value="Autorizar DLT"  class="btn btn-success" onclick="GuardarRegistro();"/></td>
            </tr>
              </tfoot>
                 ';
              }
