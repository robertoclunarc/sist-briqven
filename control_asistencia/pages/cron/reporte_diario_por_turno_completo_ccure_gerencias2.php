...
$gerencias= ['Gerencia de Mantenimiento','Gcia. de Operaciones y Procesos','Gerencia General Industrial','Gerencia de Seguridad y Salud Laboral','Gerencia de Abastecimiento','Gerencia de Talento Humano', 'Gcia de Planif Gestion e ing Ind','Gerencia Proyectos Ing. de Planta','Gcia Relaciones Instit y Desarrollo Endo']; 
...
function consultar_fichada($gerencia, $responsable,$coresponsables){
	...
	if ($turno==1){
		$complemento = 'and (CAST(fecha as datetime) =\''.date("Y-m-d",strtotime($hoy."- 1 days")).'\' and hora>\'21:15:00\' or CAST(fecha as datetime) ='.$hoy.' and hora<=\'23:59:59\')';
		...
		//and hora > \'05:15:00\' and hora  <=\'10:00:00\'';
		$complemento=' and CAST(fecha as datetime) BETWEEN \''.$hoy.'\' AND \''.$hoy.'\' 
			and ((ccure_fichadas.Direccion=\'InDirection\' and  hora > \'05:15:00\'  and hora <=\'13:00:00\') or (ccure_fichadas.Direccion=\'OutDirection\' and  hora > \'05:15:00\'  and hora <=\'16:30:00\') )';
		$sqlCtrlAcceso.= "'".$hoy." 05:15:00' AND '".$hoy." 13:00:00' ";
		$subtitulo= 'que ingresaron entre las 05:15:00 y 13:00:00 ';
	}else if ($turno==3){
		$complemento=' and CAST(fecha as datetime) BETWEEN \''.$hoy.'\' AND \''.$hoy.'\' 
			and ((ccure_fichadas.Direccion=\'InDirection\' and  hora > \'13:15:00\'  and hora <=\'23:59:00\') or (ccure_fichadas.Direccion=\'OutDirection\' and  hora > \'13:15:00\'  and hora <=\'23:59:00\') )';
	...
	$qry="SELECT Cedula, ccure_fichadas.Nombre, Cargo, TipodePersonal, Fecha_Inicidencia,CONVERT(datetime,Fecha) AS fecha, Hora,        
		...
		WHERE dbo.ccure_fichadas.Cedula=dbo.ADAM_DATOS_PERSONALES.Trabajador 
		 ".$complemento."
		AND RTRIM(GERENCIA)='".$gerencia."'
		order by GERGRAL desc,dbo.ADAM_DATOS_PERSONALES.GERENCIA, Cedula, dbo.ADAM_DATOS_PERSONALES.DESC_CCOSTO, ccure_fichadas.Direccion ASC ";
    ...
    if($contar == 0){
       $inpt = "No se han encontrado resultados!";
        ...
        while($row = $stmt1->fetch(PDO::FETCH_ASSOC, PDO::FETCH_ORI_NEXT)){
			...
			*/	
			if ($trabajador_anterior!=$row['Cedula'] ){
				$trabajador_anterior=$row['Cedula'];
				if($direccion_anterior=='ENTRADA' and $row['direction']=='SALIDA'){
					   //$inpt .='<td>2-'.$row['Hora'].'</td></tr>';
					$inpt .='<td>2-'.$hora_anterior.'</td></tr>'; 
				}elseif($direccion_anterior=='SALIDA' and $row['direction']=='SALIDA'){
					   $inpt .='<td>21-'.$hora_anterior.'</td></tr>';
						//$hora_anterior   = $row['Hora'];
				}else{
						if ($cantidadVeces>1)
					        $inpt .='<td>3-'.$hora_anterior.'</td></tr>'; 
				}	
				if ($anterior==$row['GERENCIA']){
					...
					if($direccion_anterior=='SALIDA' and $row['direction']=='ENTRADA'){
						$inpt .='<td>5-'.$row['Hora'].'</td>';
					}elseif($direccion_anterior=='ENTRADA' and $row['direction']=='SALIDA'){
						$inpt .='<td>6-</td><td>'.$row['Hora'].'</td></tr>';
					}else
						$inpt .='<td>7-'.$row['Hora'].'</td>';                       
					$total=$total+1;
				}else{
					if($direccion_anterior=='ENTRADA' and $row['direction']=='SALIDA'){
					   $inpt .='<td>8'.$row['Hora'].'</td></tr>';
					}else{
					    $inpt .='<td>9-</td></tr>'; 
					}
					...
					$inpt .='<tr style="background-color:#dadae4;">';
					$inpt .='<td>'.$contador.'</td>';
					$inpt .='<td>'.$cedula_anterior.'</td>';
					$inpt .='<td>'.$nombre_anterior.'</td>';
					$inpt .='<td>'.$cargo_anterior.'</td>';

					if($direccion_anterior=='ENTRADA' and $row['direction']=='SALIDA'){
						$inpt .='<td>10-'.$row['Hora'].'</td></tr>';
					}elseif ($direccion_anterior=='SALIDA' and $row['direction']=='SALIDA'){
						$inpt .='<td></td><td>11-'.$row['Hora'].'</td></tr>';
					}
								$contador=1;	
				} //if ($anterior==$row['GERENCIA'])
			}else{
		...
		/***************************/
		if ($direccion_anterior=='SALIDA'){
						$inpt .='<td>11-'.$hora_anterior.'</td></tr>';
		}
		/**************************/
		if ($contador==0 && isset($cedula_anterior)){
		//if (isset($cedula_anterior)){
			$inpt .='<td>1</td>';
			$inpt .='<td>'.$cedula_anterior.'</td>';
			$inpt .='<td>'.$nombre_anterior.'</td>';
			$inpt .='<td>'.$cargo_anterior.'</td>';
			$inpt .='<td>12-'.$hora_anterior.'</td>';
			$inpt .='</tr>';
			$inpt .='<tr><td colspan="6"><b>Total por '.$anterior.': 1</b></td></tr>';
			$total=$total+1;
		}else{
...
//    ENVIAR_CORREO($cuerpo,$asunto,"","", "","tecnologiaeinformacionbriqven@gmail.com","informatico12@gmail.com");
		...
		//ENVIAR_CORREO('<p>&nbsp;</p>Favor Revisar Archivo Adjunto.',$asunto,$dpf,"","","informatico12@gmail.com","");
		//ENVIAR_CORREO($cuerpo,$asunto,"","","","informatico12@gmail.com","");
		//unlink($dpf);
