...
$responsables= ['matbab','brizan','matblx','matmme','matson', 'matzag','matpud','matpud','matedr','matcom','brilci','brilci'];	

$coresponsables= ['mantenimientogerencia75@gmail.com','lizgerenciaoperaciones@gmail.com','matgas@briqven.com.ve', '', '', '','', '','','matrol@briqven.com.ve','',''];
...
function consultar_fichada($gerencia, $responsable,$coresponsables){
		...
		FROM public.acceso_personal_propio a inner join v_trabajadores v on v.trabajador=a.cedula WHERE a.tipo_personal='PROPIO' AND a.direccion='ENTRADA' AND departamento like '%".$gerencia."%' AND to_char(fecha_acceso, 'YYYY-mm-dd hh24:mi:ss') BETWEEN ";
        */

        $sqlCtrlAcceso="SELECT a.cedula::int, to_char(fecha_acceso, 'hh:mi:ss') as hora_acceso, a.nombres, a.cargo, a.departamento, a.jefe_inmediato, a.turno, v.fkunidad, b.gerencia
...
WHERE a.tipo_personal='PROPIO' 
AND a.direccion='ENTRADA' AND gerencia like '%".$gerencia."%' AND to_char(fecha_acceso, 'YYYY-mm-dd hh24:mi:ss') BETWEEN ";
		...
		if ($turno==1){
			...
			$sqlCtrlAcceso.= "'".$hoy." 05:15:00' AND '".$hoy." 10:00:00' ";
			$subtitulo= 'que ingresaron entre las 05:15:00 y 10:00:00 ';
		}else if ($turno==3){
			$complemento='and CAST(fecha as datetime) BETWEEN \''.$hoy.'\' AND \''.$hoy.'\'
			and hora > \'13:15:00\' and hora  <=\'16:00:00\'';
			$sqlCtrlAcceso.= "'".$hoy." 13:15:00' AND '".$hoy." 16:00:00' ";
			$subtitulo= 'que ingresaron entre las 13:15:00 y 16:00:00 ';
		}
		...
		AND RTRIM(GERENCIA) like '%".$gerencia."%'
		order by GERGRAL desc,dbo.ADAM_DATOS_PERSONALES.GERENCIA, dbo.ADAM_DATOS_PERSONALES.DESC_CCOSTO, Fecha_Inicidencia desc";
	   ...
	   $inpt = '<table width="100%" border="1">';
		$inpt = $inpt.'<thead>
					...
					<td colspan="5" align="center">
					    <h2><b>Reporte de entrada del personal del turno: '.$turno.', del '.date("d-m-Y", strtotime($hoy)).' de la Gerencia: '.encabezadoTabla($gerencia).', <br>'.$subtitulo.'</b></h2>
					</td>
				    </tr>
					</thead>
						</table>
    ...
    if($contar == 0){
      ...
      while($row = $stmt1->fetch(PDO::FETCH_ASSOC, PDO::FETCH_ORI_NEXT)){
		...
		echo $inpt; 
/*
		$asunto="Reporte de Entrada por Gerencia del: ".date("d-m-Y", strtotime($hoy)).", Turno:".$turno;
		$cuerpo=$inpt;
		...
		if ($total>0){
		    ...
		    if ($gerencia=='Gcia. de Operaciones y Procesos'){
		    	ENVIAR_CORREO($cuerpo,"Reporte de Asistencia de la ".$gerencia,"","", "",$JefePlantaAsistio."@briqven.com.ve",'');
		    }
		}
		*/
}    
