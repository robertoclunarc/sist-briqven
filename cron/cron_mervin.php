...
$adam_Trabajadores="SELECT LTRIM(RTRIM(trabajador)) as trabajador, 
       ...
       titulo_profesional, registro_fiscal
...
/*---------------------------------------------------------------------------------------------- */
/*----------Migrar de adam vista vw_dotacion_briqven_02_mas a postgresql----------------------- */
function cambio_cuadrilla( $cn_sitt, $cn_asistencia_laboral,$fecha_mervin){
try{
    ...
    //$fecha_a_consultar='2023-07-06 00:00:00.000'; 
    $fecha_a_consultar=$fecha_mervin; 
    ...
    $rslt="Finalizado Con Exito! el cambio de cuadrilla de: ".$cantidad_update." registros. para la fecha: ".$fecha_a_consultar;
...
/*---------------------------------------------------------------------------------------------- */
/*----------Migrar de adam vista vw_dotacion_briqven_02_mas a postgresql----------------------- */
function agregar_sustituciones($cn_sitt, $cn_asistencia_laboral, $fecha_mervin){
    $fecha_ultima_generacion=0;
    $hoy=date('Y-m-d'); 
    #CONSULTAMOS HASTA QUE FECHA ESTA CREADA LA ULTIMA HOJA DE TIEMPO
    $query_variables_sistema="select Ultima_Generacion_ht from SW_Variables_sistema svs ";
    ...
    //$fecha_a_consultar='2023-09-06 00:00:00.000'; 
    $fecha_a_consultar=$fecha_mervin; 
      ...
      { 
          ...
          CASE
                 ...
                 END  as Salida_Esperada2,  COUNT(*) 
                   ...
                   where a.fecha = '".$fecha_a_consultar."' 
                   AND cast(sistema_horario as integer) = ".$SH." and (Entrada_Esperada1!='VV:VV' and Entrada_Esperada1!='RR:RR' and Entrada_Esperada1!='PP:PP' and Entrada_Esperada1!='CS:CS' and Entrada_Esperada1!='SD:SD') GROUP BY a.Entrada_Esperada1, a.Salida_Esperada1, a.Entrada_Esperada2, a.Salida_Esperada2  order by COUNT(*) DESC"; 
print "<br>*".$fila['sustituto']."-".$query1."*";
          $stmt1      = $cn_sitt->query($query1);
          $row        = $stmt1->fetch(PDO::FETCH_ASSOC, PDO::FETCH_ORI_NEXT);
          $entEsp1    = $row['Entrada_Esperada1'];
          $salEsp1    = $row['Salida_Esperada1'];
...
guardar_log($msj1);
*/
$msj1= "10. Cambio de cuadrilla". "\n";
guardar_log($msj1,$fecha_mervin);
$msj1=cambio_cuadrilla($cn_sitt,$cn_asistencia_laboral,$fecha_mervin);
$msj1 = "Resultado: ".$msj1;
echo $msj1;
guardar_log($msj1,$fecha_mervin);

$msj1= "11. Cambio de esperanza por sustituciones". "\n";
guardar_log($msj1,$fecha_mervin);
$msj1=agregar_sustituciones($cn_sitt,$cn_asistencia_laboral,$fecha_mervin);
$msj1 = "Resultado: ".$msj1;
guardar_log($msj1,$fecha_mervin);
/*
...
function guardar_log($msj,$fecha_mervin){
$nombre_archivo = "logs.txt"; 
$html='<body bgcolor="black"><font color="white" face="Comic Sans MS,arial">';
if($archivo = fopen($nombre_archivo, "a"))
    {
        if(fwrite($archivo, $msj. "\n"))
        {
            echo $html." - ".$fecha_mervin." - ".$msj. "\n<br><br>";
        }
        else
        {
