...
function ventanaAct(idp){ 
 ...
 function ventanamonto(idp){ 
    var posicion_x; 
    var posicion_y; 
    posicion_x=(screen.width/2)-(800/2); 
    posicion_y=(screen.height/2)-(400/2);   
    var ventana = window.open('./monto/index.php?ced='+idp, "Actualizacion", "width=800,height=400,menubar=NO,toolbar=NO,directories=NO,scrollbars=NO,resizable=NO,left="+posicion_x+",top="+posicion_y+"");
    if(!ventana.focus()){
  ventana.focus();
  $(document).ready(function(){
  $.blockUI();
			...
			<th>Estatus</th> 
                ...
                <tfoot>
                  ...
                  while($reg = pg_fetch_array($listado, null, PGSQL_ASSOC))    
                   { 
				echo '<tr>';
				echo '<td><A id="act'.$reg['cedula'].'" onclick="ventanaAct('.$reg['cedula'].')" title="Actualizacion del Trabajador '.$reg['cedula'].'" href="#"><IMG SRC="images/act.png" WIDTH="20" HEIGHT="20"></A>
          <A id="act'.$reg['cedula'].'" onclick="ventanamonto('.$reg['cedula'].')" title="Monto Actual a Pagar" href="#"><IMG SRC="images/informacion.png" WIDTH="20" HEIGHT="20"></A>
        </td>';
