...
a.Horas_Dlt, a.Sustitucion, a.Cedula_Sustituido, c.nombres as sustituido, c.DESC_PUESTO as puestosustituido, a.Causal_Sustitucion,  d.desc_causa, a.Autorizado1, a.Autorizado2, e.siglado, Fecha_autor1, Fecha_autor2, b.TURNO, b.CLASE_NOMINA, b.RELACION_LABORAL
...
/**********Carga posibles codigos de ausencia*******************************************/
        $stm2 = $cn->prepare("EXEC SW_con_ausencias_posibles_ht ?, ?, ?");
            $stm2->bindParam(1, $row['CLASE_NOMINA'], PDO::PARAM_STR,2);
            $stm2->bindParam(2, $row['RELACION_LABORAL'], PDO::PARAM_STR,1);
            $stm2->bindParam(3, $row['TURNO'], PDO::PARAM_INT,10);
            $stm2->execute();
            $selected="";
            $optCodHoras=$row['cod_ausencia'] <> '' ? "<option value='NULL'>Seleccione Cod. Ausencia</option>" : "<option selected value='NULL'>Seleccione Cod. Ausencia</option>";
             while ($fila = $stm2->fetch()) {
                if ($row['cod_ausencia']==$fila['Cod_Adam']){
                    $selected="selected";
                }
                $optCodHoras.="<option ".$selected." value='".$fila['Cod_Adam']."'>".$fila['Cod_Adam'].": ".$fila['DESC_COD_HORA']." (".$fila['PAGO'].")</option>";
                $selected="";
             }
             $stm2=NULL;
/***********************************************************************************/            
            ...
            $opcionesComida='<div class="radio">
              ...
              $inpt.= '<tbody>
                        ...
                        <tr style="font-family: Arial; font-size: 8pt;">
                            ...
                            <select class="form-control" id="cod_ausencia" name="cod_ausencia" >
                                '.$optCodHoras.'
                            </select>

                            <label class="form-control">'.$row['cod_ausencia'].'</label></th>               
