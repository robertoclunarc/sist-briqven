...
session_start();
    ...
    elseif ($cbostatus==2)
        $qry.=" and (((validado_stdlt =0 or validado_stdlt is NULL) and rechazado_stdlt =0) or (validado_stdlt =0 and autorizado1 is null))"; //$qry.="  and validado_stdlt =0 and rechazado_stdlt =0 ";
    elseif ($cbostatus==3)
	        ...
	        <tbody>';
		      ...
		      while ($row=ejecutar_fetch_array($result)){
	                    //$horas_te=RestarHoras($row['inicio_st1'],$row['fin_st1']);
		      	        ...
		      	        $responsbale_carga = ''; 
                        if ($row['ced_st'] != NULL)
                            $responsbale_carga = nombre_trabajadores($row['ced_st']);
						...
						*/
						...
						if ($row["autorizado1"]!=''){
	                    	...
	                    	if ($row["validado_stdlt"]!='' && $row["validado_stdlt"]!='NULL' && $row["validado_stdlt"]!='0' ){
	                    		...
	                    		if($row["rechazado_stdlt"]!=0 && ($row["fecha_autor2"]  < $row["fecha_validado_stdlt"])){
        	      ...
        	      if ($contar>0){ 
		              $inpt .=' </tbody>
		               <!--<tfoot>
		                    <tr>
		                       <td colspan="18" align="center"> <INPUT id="cmdGuardar" type="button" value="Verificar Horas extras"  class="btn btn-success" onclick="GuardarRegistro();"/></td>
				    </tr>
		  	      </tfoot>-->
               	 ';
	            }
