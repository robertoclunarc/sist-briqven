...
a.entrada_esperada1, a.salida_esperada1, a.Entrada_Esperada2 as ent_esp2, a.salida_esperada2 as sal_esp2, a.coderror as codigo_error, a.cod_ausencia, a.horasnetapresencia, a.horasnetaausencia, b.sistema_horario, b.desc_puesto as descripcion_puesto, b.ccosto, b.detalle_ccosto as desc_ccosto, a.PagoComida,  a.Inicio_ST1 as st1_inicio,  a.Fin_St1 as st1_fin, a.horas_st as st_hora, a.Inicio_ausencia as ausencia_inicio,  a.Fin_ausencia as ausencia_fin, a.Autorizado1, a.Inicio_DLT1 as dlt1_inicio, a.Fin_DLT1 as dlt1_fin, a.Horas_Dlt as dlt_hora, a.Sustitucion, a.Cedula_Sustituido, a.Causal_Sustitucion,  a.autorizado1, a.autorizado2, c.e_mail, Fecha_autor1, Fecha_autor2, f.Observaciones as motivo, a.ced_dlt, a.ced_st, validado_stdlt, rechazado_stdlt, g.observacion
from sw_hoja_de_tiempo_real a
 INNER JOIN adam_vw_dotacion_briqven_02_mas b on a.cedula = cast(b.trabajador as integer)
 left join SW_OBSERVACIONES_STDLT f on a.cedula = f.cedula and a.fecha = f.fecha
 left join sw_observaciones_rechazo_stdlt g on a.cedula=  g.cedula and a.fecha = g.fecha_stdlt
 left join trabajadores c on cast(a.Cedula_Sustituido as integer) = cast(c.trabajador as integer) 
 where a.fecha = '".$fecha."' and a.cedula=".$trabajador.' order by g.fecha desc'; 
...
//print $b;
       ...
       // CONSULTAMOS LA CANTIDAD DE HORAS DE SOBRE TIEMPO SEMANAL
        ...
        if ($contar>0){ 
            ...
            $cn_sitt                = Conectarse_sitt();
            $stmt1_sitt             = $cn_sitt->query($a);
            $contar_sitt            = $stmt1_sitt->columnCount(); 
            $contar_row_sitt        = $stmt1_sitt->rowCount(); 
            $row_sitt               = $stmt1_sitt->fetch(PDO::FETCH_ASSOC, PDO::FETCH_ORI_NEXT);   
            $entrada_esperada1_sitt = $row_sitt['entrada_esperada1'];
            $salida_esperada1_sitt  = $row_sitt['salida_esperada1'];
            $Entrada_Esperada2_sitt = $row_sitt['ent_esp2'];
            $Salida_Esperada2_sitt  = $row_sitt['sal_esp2'];
            $st1_inicio_sitt        = $row_sitt['st1_inicio'];
            $st1_fin_sitt           = $row_sitt['st1_fin'];
            $st_hora_sitt           = $row_sitt['st_hora'];  
            if ($Entrada_Esperada2_sitt =='00:00'){          
                $Entrada_Esperada2_sitt = '';
            }
            if ($Salida_Esperada2_sitt =='00:00'){          
                $Salida_Esperada2_sitt = '';
            }

            if ($entrada_esperada1_sitt =='LL:LL' || $entrada_esperada1_sitt =='FF:FF'){          
                $type2 = '';
            }else{
                $type2 = 'type="time" ';
            }    
        ...
        if($contar == 0){
            ...
            if ($row["autorizado1"]!=''){
                ...
                if ($row["validado_stdlt"]!='' && $row["validado_stdlt"]!='NULL' && $row["validado_stdlt"]!='0' ){
                    $status_validado='<i class="fa fa-check" aria-hidden="true"></i>';
                    $status = 'Verificado'; 
                }else{
                    if ($row['rechazado_stdlt']!='' && $row["rechazado_stdlt"]!='NULL' && $row["rechazado_stdlt"]!='0' ){
                        $status_validado='Rechazado';
                    }    
                    if ($row["autorizado2"]!=''){
            ...
            if (($row['autorizado1'] =='') || ($row['autorizado1'] =='NULL')){
              ...
              $inpt.= '<tbody>
                        ...
                        ';
                        if ($entrada_esperada1_sitt!=$row['entrada_esperada1']){
                        $inpt.= '<tr style="font-family: Arial; font-size: 8pt;">
                            <th  width="7%"  class="success">Ent. Esp. 1<br>Actual</th>
                            <th  width="9%"  class=""><input id="entEsp1_sitt" '.$type2.' name="entEsp1_sitt" class="form-control" value="'.$entrada_esperada1_sitt.'" readonly></th>              
                            <th  width="7%" class="success">Sal. Esp. 1<br>Actual</th>
                            <th  width="9%" class=""><input id="salEsp1_sitt" '.$type2.' name="salEsp1_sitt" class="form-control" value="'.$salida_esperada1_sitt.'" readonly></th>
                            <th  width="7%" class="success">Ent. Esp. 2<br>Actual</th>
                            <th  width="9%" class=""><input id="entEsp2_sitt" '.$type2.' name="entEsp2_sitt" class="form-control" value="'.$Entrada_Esperada2_sitt.'" readonly></th>              
                            <th  width="7%" class="success">Sal. Esp. 2<br>Actual</th> 
                            <th  width="7%" class=""><input id="salEsp2_sitt" '.$type2.' name="salEsp2_sitt" class="form-control" value="'.$Salida_Esperada2_sitt.'" readonly></th>                                         
                            <th  width="5%" class="success">
                            <div id="sp20"></div>
                            </th>
                        ...
                        <tr style="font-family: Arial; font-size: 8pt;">
                            ...
                            <th class=""><input id="entReal1" '.$type.' name="entReal1" class="form-control" value="'.$row['entrada_real1'].'" readonly>
                            </th>              
                            <th class="danger">Sal. Real 1</th>
                            <th class=""><input id="salReal1" '.$type.' name="salReal1" class="form-control" value="'.$row['salida_real1'].'" readonly></th>  
                            <th class="danger">Ent. Real 2</th> 
                            <th  class=""><input id="entReal2" '.$type.' name="entReal2" class="form-control" value="'.$row['entrada_real2'].'" readonly></th>             
                            <th class="danger">Sal. Real 2</th>
                            <th  class=""><input id="salReal2" '.$type.' name="salReal2" class="form-control" value="'.$row['salida_real2'].'"  readonly></th>
                            <th  class="danger">
                            <div id="sp1"></div>
                     ...
                     $inpt.= '
                        ...
                        <tr style="font-family: Arial; font-size: 8pt;">
                            <th colspan="5"><textarea  readonly name="motivo_ST" placeholder="Motivo de las horas extras" rows="2" cols="100%">'.$motivo_st.'</textarea></th>
                     ...
                     $inpt.= '<tr style="font-family: Arial; font-size: 8pt;">
                            <th class="danger" rowspan="2">Validar o <br>Rechazar</th>

                            
                        </tr>
                        <tr style="font-family: Arial; font-size: 8pt;">
                            <th colspan="5"><textarea name="observacion_validar" placeholder="En caso de rechazar esta solicitud, ingrese el motivo" rows="2" cols="100%">'.$row["observacion"].'</textarea></th>
                            <th class="danger">status Validado</th>
                            <th class="">'.$status_validado.'</th> 
                            <th class="danger" rowspan="2">
