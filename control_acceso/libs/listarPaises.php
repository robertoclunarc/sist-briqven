...
function anular(idm,tipomov,estatus){
	...
	var control;
	if (r != null){
	    dir_url = "anular_movimiento.php?idm="+idm+"&tipomov="+tipomov+"&sta=ANULADO&r="+r;
	    if (estatus=='SOLICITADO')
	        {control="pend_"+idm; document.getElementById(control).innerHTML="";}
	    if (estatus=='CONFORMADO')
	        {control="sol_"+idm; document.getElementById(control).innerHTML="";}
...
if ($_SESSION['nivel_ca']==1)
     $query="SELECT v_movimientos_1.*, nombres_agr, unidades_agr FROM v_movimientos_1, agrupamiento_operaciones WHERE idmovimiento=fkmovimiento_agr and nivel = ".$_SESSION['nivel_ca']." and estatus not in ('PENDIENTE','SOLICITADO');";
...
elseif ($_SESSION['nivel_ca']==7)
     $query="SELECT v_movimientos_1.*, nombres_agr, unidades_agr FROM v_movimientos_1, agrupamiento_operaciones WHERE idmovimiento=fkmovimiento_agr and estatus in ('VALIDADO');";         
   else 
                ...
                <tbody>                     
                  ...
                  while($reg = pg_fetch_array($listado, null, PGSQL_ASSOC))    
                   ...
                   <?php
                        ...
                        $ciclosImp = array('SALIDA AUTORIZADO', 'ENTRADA AUTORIZADO');
                        if (exist_usuario($idm, $_SESSION['user_session_ca'], $cn) && $reg['estatus']!='ANULADO' && in_array($reg['ciclo'], $ciclosImp)){
                    ?>                          
